var RoomsView = {
  ...
  initialize: function() {
    ...
    messages.forEach(message => { //for each message object, 
      ...
      if (!RoomsView.allRooms.includes(roomName)) { //if the allRooms array does not already has a given roomname
  ...
  showRoom: function(event) {
    var roomMessages = App.fetch(el => el.roomname === event.target.value);
